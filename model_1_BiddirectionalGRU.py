...
#
embedding_matrix = embedding_matrix_fasttext*0.4 + embedding_matrix_glove*0.6

### Training via Word Embedding
inputs = Input(shape=(200,))
embedding = Embedding(input_dim=30000, output_dim=300, weights=[embedding_matrix],trainable = True)(inputs)
embedding = SpatialDropout1D(0.2)(embedding)
# model = Bidirectional(LSTM(40,return_sequences = True, activation='relu', dropout=0.1,recurrent_dropout=0.1))(embedding)
...
###
concat = Concatenate(axis=1)([temp_max,dense])
...
for train_index, test_index in kf.split(train_seq,target):
    model.fit([train_seq[train_index],train_preprocessing.values[train_index]],target.values[train_index],epochs=5,batch_size=1024,\
